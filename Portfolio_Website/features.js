...
const projectExtraData = {
  ...
  'RakhtSetu - रक्त सेतु': {
      category: 'Cloud / AWS',
      tech: ['Node.js', 'AWS Lambda', 'DynamoDB', 'API Gateway', 'S3', 'Amazon SNS', 'EmailJS', 'Serverless Framework'],
      features: [
...
function initFeatures() {
  ...
  document.querySelectorAll('#work1 .card').forEach(function (card) {
    card.style.position = 'relative'; // needed for the ::after badge

    card.addEventListener('click', function (e) {
      // Don't open modal if user clicked the "View Repository" button
      if (e.target.tagName === 'BUTTON' || e.target.closest('button')) return;

      // Extract data from the card's existing DOM
